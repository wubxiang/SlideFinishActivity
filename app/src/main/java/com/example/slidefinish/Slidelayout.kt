...
class Slidelayout @JvmOverloads constructor(
     ...
     */
    val EDGE_LEFT: Int = ViewDragHelper.EDGE_LEFT

    /**
     * Edge flag indicating that the right edge should be affected.
     */
    val EDGE_RIGHT: Int = ViewDragHelper.EDGE_RIGHT

    /**
     * Edge flag indicating that the bottom edge should be affected.
     */
    val EDGE_BOTTOM: Int = ViewDragHelper.EDGE_BOTTOM
     ...
     * fling/snap.
     */
    val STATE_IDLE: Int = ViewDragHelper.STATE_IDLE

    /**
     * A view is currently being dragged. The position is currently changing as
     * a result of user input or simulated user input.
     */
    val STATE_DRAGGING: Int = ViewDragHelper.STATE_DRAGGING

    /**
     * A view is currently settling into place as a result of a fling or
     * predefined non-interactive motion.
     */
    val STATE_SETTLING: Int = ViewDragHelper.STATE_SETTLING
    ...
    init {
        ...
        val a = context.obtainStyledAttributes(attrs, R.styleable.Slidelayout)
        val edgeSize = a.getDimensionPixelSize(R.styleable.Slidelayout_edge_size, -1)
        if (edgeSize > 0) setEdgeSize(edgeSize)
        val mode = EDGE_FLAGS[a.getInt(R.styleable.Slidelayout_edge_flag, 0)]
        setEdgeTrackingEnabled(mode)
        val shadowLeft = a.getResourceId(
            R.styleable.Slidelayout_shadow_left,
            R.drawable.shadow_left
        )
        val shadowRight = a.getResourceId(
            R.styleable.Slidelayout_shadow_right,
            R.drawable.shadow_right
        )
        val shadowBottom = a.getResourceId(
            R.styleable.Slidelayout_shadow_bottom,
...
//        mDragHelper.setMaxVelocity(minVel * 2f)
        ...
        addSwipeListener(SwipeBackListenerActivityAdapter(context as Activity))
    }
...
//        mDragHelper.setSensitivity(context, sensitivity)
     ...
     * callback's
     * [me.imid.Slidelayout.lib.ViewDragHelper.Callback.onEdgeTouched]
     * and
     * [me.imid.Slidelayout.lib.ViewDragHelper.Callback.onEdgeDragStarted]
...
//        mDragHelper.setEdgeSize(size)
    ...
    private inner class ViewDragCallback() : ViewDragHelper.Callback() {
        ...
        override fun tryCaptureView(child: View, pointerId: Int): Boolean {
            val ret: Boolean = mDragHelper.isEdgeTouched(mEdgeFlag, pointerId)
            if (ret) {
                ...
                if (!mListeners.isNullOrEmpty()) {
                    for (listener: SwipeListener in mListeners!!) {
        ...
        override fun onViewReleased(releasedChild: View, xvel: Float, yvel: Float) {
            ...
            if ((mTrackingEdge and EDGE_LEFT) != 0) {
                left = if (xvel > 0 || xvel == 0f && mScrollPercent > mScrollThreshold) (childWidth
                        + mShadowLeft.getIntrinsicWidth() + OVERSCROLL_DISTANCE) else 0
            } else if ((mTrackingEdge and EDGE_RIGHT) != 0) {
                left =
                    if (xvel < 0 || xvel == 0f && mScrollPercent > mScrollThreshold) -((childWidth
                            + mShadowLeft.getIntrinsicWidth() + OVERSCROLL_DISTANCE)) else 0
            } else if ((mTrackingEdge and EDGE_BOTTOM) != 0) {
                top =
                    if (yvel < 0 || yvel == 0f && mScrollPercent > mScrollThreshold) -((childHeight
                            + mShadowBottom.getIntrinsicHeight() + OVERSCROLL_DISTANCE)) else 0
            }
